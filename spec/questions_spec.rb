...
describe 'the Friday test :)' do
  ...
  it 'do the modulo' do
    number = 9
    divisor = 2
    expect(my_modulo number, divisor).to eq 1
  end

  it 'is divisible by 3' do
    number = 9
    expect(is_divisible_by_3?(number)).to eq true
  end

   it 'is not divisible by 3' do
     number = 8
    expect(is_divisible_by_3?(number)).to eq false
  end

    it 'is divisible by 5' do
    number = 30
    expect(is_divisible_by_5?(number)).to eq true
  end

   it 'is not divisible by 5' do
     number = 32
    expect(is_divisible_by_5?(number)).to eq false
  end

    it 'is divisible by 15' do
    number = 30
    expect(is_divisible_by_15?(number)).to eq true
  end

   it 'is not divisible by 15' do
     number = 33
    expect(is_divisible_by_15?(number)).to eq false
  end

  it 'do fizz if it is divisible by 3' do
    number = 6
    expect(fizzbuzz_without_modulo(number)).to eq "fizz"
  end

  it 'do not do fizz if it is not divisible by 3' do
    number = 7
    expect(fizzbuzz_without_modulo(number)).to eq number
  end

    it 'do buzz if it is divisible by 5' do
    number = 10
    expect(fizzbuzz_without_modulo(number)).to eq "buzz"
  end

  it 'do not do buzz if it is not divisible by 5' do
    number = 8
    expect(fizzbuzz_without_modulo(number)).to eq number
  end

  it 'do fizzbuzz if it is divisible by 15' do
    number = 45
    expect(fizzbuzz_without_modulo(number)).to eq "fizzbuzz"
  end

  it 'do not do fizzbuzz if it is not divisible by 15' do
    number = 38
    expect(fizzbuzz_without_modulo(number)).to eq number
  end
